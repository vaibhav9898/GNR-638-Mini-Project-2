...
# Define paths
csv_file_path = '/home/ninad/vaibhav_r/shubh/image_pairs_4000.csv'
...
for epoch in range(num_epochs):
    for batch_idx, (images, sharp_imgs) in enumerate(data_loader):
        ...
        # Calculate PSNR
        image = (outputs[0] * 255).cpu().detach().numpy().astype(np.uint8)
        sharp_img_np = (sharp_imgs[0] * 255).cpu().detach().numpy().astype(np.uint8)
        psnr = peak_signal_noise_ratio(image, sharp_img_np)

        # Append loss and PSNR to lists
        train_losses.append(loss.item())
        train_psnrs.append(psnr)

        # Print statistics
        if batch_idx % 10 == 0:
            print(f'Epoch: {epoch}/{num_epochs}, Step: {batch_idx}/{len(data_loader)}, Train Loss: {loss.item():.3f}, Train PSNR: {psnr:.3f}')

    # Evaluation on test set
    test_psnr_epoch = 0
    num_samples = len(test_dataset)
    for i in range(num_samples):
        test_blur, test_sharp = test_dataset[i]
        ...
        test_psnr_epoch += psnr

    test_psnr_epoch /= num_samples
    test_psnrs.append(test_psnr_epoch)
    print(f"Epoch: {epoch}/{num_epochs}, Test PSNR: {test_psnr_epoch:.3f}")
...
plt.plot(test_psnrs, label='Testing PSNR')
plt.xlabel('Epoch')
plt.ylabel('PSNR')
